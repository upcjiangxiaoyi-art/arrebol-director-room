...
/*
 * Arrebol Director Room 暗河红霞 Arrebol D v1.0.3.2
 * 抽屉内嵌稳定版：
 * - 情感导演 / 剧情导演 双页面
 * - 双 API / 双模型 / 双预设
 * - 拉取模型
...
(function () {
    ...
    var EXT = "arrebol-d-final-v1032-drag-click-fix";
    ...
    function drawerHTML() {
        ...
        return '<div id="adr044-drawer"><div class="inline-drawer">'
            + '<div class="inline-drawer-toggle inline-drawer-header"><b>🎬 Arrebol D 暗河红霞导演系统 v1.0.3.2</b><div class="inline-drawer-icon fa-solid fa-circle-chevron-down down"></div></div>'
    ...
    function adr048PanelHTML() {
        ...
        return '<div id="adr048-popup-panel" data-open="0">'
            ...
            + '<div class="adr048-note">暗河红霞 Arrebol D 已就绪。情感/剧情可分别设置自动触发间隔；小红霞支持自由拖动，只有点击才打开面板。</div>'
        ...
        try {
            ...
            if (!btn) {
                ...
                function endDrag(ev) {
                    ...
                    if (moved) {
                        btn.__adr048JustDragged = true;
                        setTimeout(function () {
                            try { btn.__adr048JustDragged = false; } catch (e) {}
                        }, 650);
                    }

                    // 拖动结束绝不打开面板；只允许真正 click 打开。
                    try { ev.preventDefault(); ev.stopPropagation(); } catch(e) {}
                }
            ...
            if (!btn.__adr048ClickBound) {
                ...
                function hardOpen(ev) {
                    ...
                    // 刚刚发生过拖动，就吞掉这次 click / touchend / pointerup，避免误弹。
                    if (btn.__adr048JustDragged) {
                        return false;
                    }

                    setTimeout(function () {
                        try {
                            if (!btn.__adr048JustDragged) adr048OpenPopupPanel();
                ...
                // 不再用 pointerup / touchend 打开，避免拖动松手误触。
            }
        } catch (e2) {
