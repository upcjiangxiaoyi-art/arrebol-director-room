...
/*
 * Arrebol Director Room 暗河红霞 Arrebol D v1.0.4.5
 * 抽屉内嵌稳定版：
 * - 情感导演 / 剧情导演 双页面
 * - 双 API / 双模型 / 双预设
 * - 拉取模型
...
(function () {
    ...
    var EXT = "arrebol-d-final-v1045-force-rebuild-safe-inject";
    ...
    function refreshMessageDom(index) {
        // v1.0.4.5：禁止整条 innerHTML 重写，避免破坏其他美化。
        return false;
    }
    ...
    function drawerHTML() {
        ...
        return '<div id="adr044-drawer"><div class="inline-drawer">'
            + '<div class="inline-drawer-toggle inline-drawer-header"><b>🎬 Arrebol D 暗河红霞导演系统 v1.0.4.5</b><div class="inline-drawer-icon fa-solid fa-circle-chevron-down down"></div></div>'
    ...
    function adr048PanelHTML() {
        ...
        return '<div id="adr048-popup-panel" data-open="0">'
            ...
            + '<div class="adr048-note">暗河红霞 Arrebol D 已就绪。情感/剧情可分别设置自动触发间隔；纯文本标记注入；强制重建面板，修复更新后旧按钮失活；设置稳定保存。</div>'
    ...
    function adrDForceRemoveOldDom() {
        try {
            var d = rootDoc();
            [
                "#adr044-drawer",
                "#adr048-popup-panel",
                "#adr048-popup-backdrop",
                "#adr048-floating-entry",
                "#adr048-ipe-anchor-entry",
                "#adr046-quick-entry",
                "#adr0461-doorbell-right",
                "#adr0461-doorbell-left",
                "#adr0462-ipe-entry"
            ].forEach(function (sel) {
                try {
                    var nodes = Array.prototype.slice.call(d.querySelectorAll(sel));
            ...
            // 兜底删除所有疑似红霞旧按钮/旧面板，避免更新后旧 DOM 抢事件。
            try {
                Array.prototype.slice.call(d.querySelectorAll("[id^='adr048-'], [id^='adr046-'], [id^='adr044-']")).forEach(function (node) {
                    if (node && node.parentNode) node.parentNode.removeChild(node);
                });
            } catch (e2) {}
        } catch (e3) {
            console.warn("[Arrebol D] force remove old dom failed", e3);
        }
    }
    ...
    function init() {
        ...
        try {
            settings();
            adrDForceRemoveOldDom();
            mountDrawer();
            installProbeGlobals();
            installProbeDelegation();
            try { rootWin().adrDStableAutoSaveBound = false; } catch (eBindReset) {}
            bindDirect();
            adr048CreatePopupPanel();
